...
<table style="width:80%" class="table table-bordered table-hover">
 <div id="bulkOptionContainer" class="col-xs-6 col-xs-offset-3">
  ...
  <?php
      ...
      if(isset($_GET['naziv_zavoda'])){
		  
		  $the_naziv_zavoda =$_GET['naziv_zavoda'];
		  
	  }
  
  
   $sql =<<<EOF
      SELECT  * from djelatnici WHERE naziv_zavoda LIKE '$the_naziv_zavoda' ;
...
EOF;
   ...
   $db->close();
?>
<?php echo "<h2>Djelatnici kojima je naziv zavoda $naziv_zavoda</h2>" ?>