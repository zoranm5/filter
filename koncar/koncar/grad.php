...
<table style="width:80%" class="table table-bordered table-hover">
 <div id="bulkOptionContainer" class="col-xs-6 col-xs-offset-3">
  ...
  <?php
      ...
      if(isset($_GET['grad_stanovanja'])){
		  
		  $the_grad_stanovanja  =$_GET['grad_stanovanja'];
		  
	  }
  
  
   $sql =<<<EOF
      SELECT  * FROM djelatnici WHERE grad_stanovanja  LIKE '$the_grad_stanovanja' ;
...
EOF;
   ...
   $db->close();
?>
<?php echo "<h2>Djelatnici kojima je grad stanovanja $grad_stanovanja</h2>" ?>